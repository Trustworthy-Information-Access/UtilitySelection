...
if __name__ == "__main__":
    data_type = "hotpotqa"
    root = "results_first10_merge_remian/"
    query_answer = {}
    with open(data_type+"_answer.jsonl", "r", encoding="utf-8") as file:
        for line in file:
            js = json.loads(line)
            query_answer[js["query_id"]] = js["reference_answer"]

    file_list = [

        root + "llama31_"+data_type+"-answer_5_original_nw0",
        root + "llama31_"+data_type+"-answer_10_original_nw0",
        root + "llama31_"+data_type+"-answer_15_original_nw0",
        root + "llama31_"+data_type+"-answer_20_original_nw0",
        root + "llama31_"+data_type+"-answer_40_original_nw0",
        root + "llama31_"+data_type+"-answer_60_original_nw0",
        root + "llama31_"+data_type+"-answer_80_original_nw0",
        root + "llama31_"+data_type+"-answer_100_original_nw0",
    ...
    for file in file_list:
        ...
        f1_scores = []
        print(file)
        outdir = open(file+"_results.jsonl", "w", encoding="utf-8")
        infile = open(file+".jsonl", 'r', encoding='utf-8')
        for line in tqdm(infile.readlines()):
            d = json.loads(line)
            d["gt_answer"] = query_answer[d['query_id']]
            if "Answer(just one entity):" in d['answer']:
                d['EM'], d['F1'] = deal_answer(d['answer'].split("Answer(just one entity):")[1], d["gt_answer"])
            elif  "Answer:" in d['answer']:
                d['EM'], d['F1'] = deal_answer(d['answer'].split("Answer:")[1], d["gt_answer"])
            else:
                d['EM'], d['F1'] = deal_answer(d['answer'], d["gt_answer"])
        ...
        print(f"{em_scores_mean*100} \t {f1_scores_mean*100}")
        # print("em \t {}".format(em_scores_mean*100))
        # print("f1 \t {}".format(f1_scores_mean*100))
