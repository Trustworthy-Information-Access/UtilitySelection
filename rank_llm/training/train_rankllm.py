...
def initialize_training_state(train_dataloader, args, accelerator):
    ...
    # )
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / (args.per_device_train_batch_size * accelerator.num_processes * args.gradient_accumulation_steps))
...
#     # 修改后（正确）:
#     num_update_steps_per_epoch = math.ceil(len(train_dataset) / (args.per_device_train_batch_size * accelerator.num_processes * args.gradient_accumulation_steps))
...
def main():
    ...
    print(f"实际总批量大小: {accelerator.gradient_accumulation_steps * accelerator.num_processes * args.per_device_train_batch_size}")
    ...
    # )
    overrode_max_train_steps, num_update_steps_per_epoch = initialize_training_state(
        train_dataset, args, accelerator  # 添加 train_dataset 和 accelerator
    )
    starting_epoch, resume_step, completed_steps = resume_from_checkpoint(
        args, num_update_steps_per_epoch, train_dataloader
    )
    num_training_steps = args.max_train_steps  # 直接使用全局总步数
    lr_scheduler = get_scheduler(
        name=args.lr_scheduler_type,
        optimizer=optimizer,
        num_warmup_steps=args.num_warmup_steps,  # 已修正为全局步数
    ...
    if args.with_tracking:
        accelerator.end_training()  # 现在安全结束训练
