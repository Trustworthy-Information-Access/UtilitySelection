...
def get_direct_judge_list_utility_ranking(query, passages):
    num = len(passages)
    messages = get_prefix_direct_judge_list_utility_ranking(query, num)
    rank = 0
    for passage in passages:
        rank += 1
        content = passage
    ...
    return messages
def get_prefix_direct_judge_list_utility_ranking(query, num):
    return [{'role': 'user',
            'content': "You are the utility ranker, an intelligent assistant that can rank passages based on their utility in answering the question."},
            {'role': 'assistant',
            'content': "Yes, i am the ranker."},
            {'role': 'user',
            'content': f"I will provide you with {num} passages, each indicated by number identifier []. \nRank the passages based on their utility in answering the question: {query}."},
            {'role': 'assistant', 'content': 'Okay, please provide the passages.'}]

def get_post_prompt_utility_ranking(query, num):
    return f"Question: {query}. \n\n Firstly, generate the answer based on the {num} passages or your internal knowledge with one or a few sentences without the source. Then, rank the {num} passages above based on their utility in answering the question. The passages should be listed in utility descending order using identifiers.  The output format should be 'Answer: xxx; The ranked list: [] > [] > [] > ..., e.g., [i] > [j] > [k] > ...' Only response the answer and ranking results, do not say any word or explain."
...
class EncodeDataset(Dataset):
    ...
    def __getitem__(self, item) -> Tuple[str, List[int]]:
        ...
        for index, passage in enumerate(group_negatives):
            formated_passages.append(passage["text"])
            formated_passages_ids.append(passage["docid"])
        assert self.data_args.type_prompt in ["answer", "utility",  "relevance_ranking"]
        if self.data_args.type_prompt == "answer":
            ...
            messages = get_direct_judge_list_utility(query, formated_passages,answer)
        elif self.data_args.type_prompt == "relevance_ranking":
            messages = get_direct_judge_list_relevance(query,  formated_passages)
        # elif self.data_args.type_prompt == "relevance_selection":
        #     messages = get_direct_judge_list_relevance_selection(query, formated_passages)
